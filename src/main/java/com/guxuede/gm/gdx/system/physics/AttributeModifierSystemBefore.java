 ...
 */
public class AttributeModifierSystemBefore extends IteratingSystem {

    private static final Family family = Family.all(AttributeModifferComponent.class).get();

    public AttributeModifierSystemBefore(int priority){
        super(family);
    ...
    protected void processEntity(Entity entity, float deltaTime) {
        ...
        attributeArrayMap.keySet().forEach(attribute->{
            attribute.init(entity);
            Array<AttributeModifier> attributeModifiers = attributeArrayMap.get(attribute);
            attributeModifiers.forEach((v)->{
                v.beforeApply(entity);
            });
            attributeModifiers.forEach((v)->{
                attribute.plus(entity, v.getNumber(entity));
            });
        });
    }
}
